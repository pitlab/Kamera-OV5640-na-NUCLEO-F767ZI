//////////////////////////////////////////////////////////////////////////////
//
// Moduł obsługi wyświetlacza TFT 320x240 dla płytki Nucleo-F767ZI
//
// (c) PitLab 2019
// http://www.pitlab.pl
//////////////////////////////////////////////////////////////////////////////
...
{
	GPIOF->BSRR = (1<<(LCD_CS+CLEAR));							//PF3=CS=0
	GPIOC->BSRR = (1<<(LCD_RS+CLEAR)) + (1<<(LCD_WR+CLEAR));	//PC3=RS=0
	GPIOA->BSRR = (1<<LCD_RD);									//PA3=RD=1

	//dane s� rozsiane po r�nych portach, wi�c posk�adaj je
	GPIOD->BSRR = (1<<(15+((chData & 0x02) != 0x02)*CLEAR)); 	//D1 @ PD.15
	GPIOE->BSRR = (1<<(9+ ((chData & 0x40) != 0x40)*CLEAR))| 	//D6 @ PE.9
			 	  (1<<(11+((chData & 0x20) != 0x20)*CLEAR))|	//D5 @ PE.11
				  (1<<(13+((chData & 0x08) != 0x08)*CLEAR));	//D3 @ PE.13
	GPIOF->BSRR = (1<<(12+((chData & 0x01) != 0x01)*CLEAR))|	//D0 @ PF.12
			 	  (1<<(13+((chData & 0x80) != 0x80)*CLEAR))|	//D7 @ PF.13
				  (1<<(14+((chData & 0x10) != 0x10)*CLEAR))|	//D4 @ PF.14
				  (1<<(15+((chData & 0x04) != 0x04)*CLEAR));	//D2 @ PF.15
...
////////////////////////////////////////////////////////////////////////////////
// Wysy�a dane do wy�wietlacza LCD
// Parametry: chDane
// Zwraca: nic
////////////////////////////////////////////////////////////////////////////////
...
void LCD_init(void)
{
	delay(20);
	// LCD_RESET 1 - 0 - 1
	GPIOF->BSRR = (1<<LCD_RST);			//PF5=RES=1
	HAL_Delay(10);
	GPIOF->BSRR = (1<<(LCD_RST+CLEAR));	//PF5=RES=0
	HAL_Delay(20);
	GPIOF->BSRR = (1<<LCD_RST);			//PF5=RES=1
	HAL_Delay(20);
	// CS HIGH, WR HIGH, RD HIGH, CS LOW
	GPIOF->BSRR = (1<<LCD_CS);			//PF3=CS=1
	GPIOC->BSRR = (1<<LCD_WR);			//PC0=WR=1
	GPIOA->BSRR = (1<<LCD_RD);			//PA3=RD=1
...
////////////////////////////////////////////////////////////////////////////////
// generowanie op�nienia
// Parametry: nCzas - op�nienie w ms
// Zwraca: nic
////////////////////////////////////////////////////////////////////////////////
void delay(unsigned int nCzas)
{
	//volatile int x, y, count_ms = 2942;
	unsigned int x, y, count_ms = 2942;

	for (x=0; x<nCzas; x++)
		for (y=0; y<count_ms; y++);
}
...
void drawHLine(int x, int y, int len)
{
	...
	GPIOF->BSRR = (1<<(LCD_CS+CLEAR));	// CS 0
	GPIOF->BSRR = (1<<(LCD_CS+CLEAR));	// CS 0
	setXY(x, y, x+len, y);
